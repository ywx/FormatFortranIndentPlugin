...
void CMyWxFortranIndent::myCreateFortranRegEx()
{
	...
	myFortranRegEx[wxT("regexFunction")] = new wxRegEx( wxT("^(\\s*)(((pure)|(impure))\\s+)?(((recursive)|(elemental))\\s+)?(((integer)|(real)|(complex)|(logical)|(character)|((double)(\\s*)(precision))|((type)(\\s*)\\((\\s*)([a-zA-Z0-9_]+)((\\s*)\\((\\s*)([a-zA-Z0-9_]+)(\\s*)\\))?(\\s*)\\)))((\\s*)\\((\\s*)((kind|len)(\\s*)=(\\s*))?(\\d*)((\\s*)(,)(\\s*)((kind|len)(\\s*)=(\\s*))?(\\d*)(\\s*))?(\\s*)\\))?)?(\\s*)((module)(\\s+))?(function)(\\s+)([a-zA-Z0-9_]+)(\\s*)\\((\\s*)(([a-zA-Z0-9_]+)((\\s*)(,)(\\s*)([a-zA-Z0-9_]+))*)?(\\s*)\\)"), options );
...
bool CMyWxFortranIndent::getIsHasPreprocessor( const wxString & srcLine, int iPreprocessorType )
{
    bool bRet = false;
    switch( iPreprocessorType )
    {
