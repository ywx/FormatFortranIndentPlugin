...
NOTE: It could not support the "where", "type", "block", "&".
...
Format_Fortran_Indent_Plugin.cbplugin in repository compile by MinGW32 4.6.2 with wxWidgets 2.8.12 and Code::Blocks IDE for Fortran v0.7 src.
...
注意：目前它还不支持 where, type, block 等语句；不支持 & 续行。
...
仓库上的 Format_Fortran_Indent_Plugin.cbplugin 由 wxWidgets 2.8.12 和 Code::Blocks IDE for Fortran 0.7 在 MinGW32 4.6.2 编译生成。
...
Format Fortran Indent Plugin 只是运用正则表达式对源代码进行以行为单位的模式匹配，然后进行代码缩进格式化。所以它只能支持Fortran 95, Fortran 90语法中的一个子集。目前它还不支持 where, type, block 等语句；不支持 & 续行。只能说基本可用。有空的话，我会完善它。希望大家也来关心它，培养它。更希望有人能从编译原理方向重塑它。


最后更新: March 12th 2012
